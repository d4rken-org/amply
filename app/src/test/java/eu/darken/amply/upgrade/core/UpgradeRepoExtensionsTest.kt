...
class UpgradeRepoExtensionsTest {
    ...
    private fun explodingRepo() = object : UpgradeRepo {
        override val storeSite: String = ""
        override val upgradeSite: String = ""
        override val betaSite: String = ""
    ...
    fun `a hanging refresh fails open within the supplied budget`() = runTest {
        // The timeout covers refresh + wait: starting the wait only AFTER an unbounded refresh
        // would let a hanging Play call park the gate far past the window.
        val repo = FakeRepo(pro = false, settled = false)
    ...
    fun `isProSettled errors fail open`() = runTest {
        explodingRepo().isProSettled() shouldBe true
    }
    ...
    @Test
    fun `settled non-pro denies even when the info carries an error`() = runTest {
        // isSettled is checked before any error consideration: a settled non-Pro state is a real
        // answer, so the tap routes to the upgrade screen instead of failing open.
        val repo = FakeRepo(pro = false, settled = true, error = IllegalStateException("billing broke"))
        repo.isProForUi() shouldBe false
    }
    ...
    fun `the settle decision reads ownership off the settling Info itself`() = runTest {
        // A two-step (wait on isSettled, re-read upgradeInfo) could pair the settle signal with a
        // stale replay. The Info that satisfies the settled-wait IS the decision.
        val repo = FakeRepo(pro = false, settled = false)
    ...
    @Test
    fun `isProForUi errors fail open`() = runTest {
        explodingRepo().isProForUi() shouldBe true
    }
