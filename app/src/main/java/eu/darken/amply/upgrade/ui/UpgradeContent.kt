...
/**
 * Building blocks shared by both flavors' upgrade screens. They live in `src/main` so the FOSS and
 * gplay screens — which are otherwise entirely separate source sets — render the same layout,
 * spacing and typography. Ported from SD Maid SE's `UpgradeContent`.
 */
internal object UpgradeScreenTags {
    const val LOADING = "upgrade_loading"
    const val ACTIONS = "upgrade_actions"
    const val PRO_BADGE = "upgrade_pro_badge"
...
// The app's brand title, composed through the title template so translators own the word order and
// punctuation instead of the code assuming "name, space, qualifier".
//
// The two flags are deliberately separate. `includeQualifier` decides whether the tier word is part
// of the title at all; `highlightQualifier` only decides whether it is colored. The FOSS status-free
// view needs "Amply FOSS" in plain text, so it passes (true, false) — collapsing these into one flag
// would silently drop FOSS from that screen.
@Composable
...
// Same composition for the call sites that need a plain String (settings rows take String, not
// AnnotatedString). Routed through brandTitle so the two forms cannot drift apart.
@Composable
...
// Composed app title with the flavor postfix highlighted while Pro is active.
@Composable
...
// Marker char for brand-title splicing: formatted into the translated pattern via the normal Android
// format path (so %1$s vs %s, argument reordering, and %% all behave), then replaced with the styled
// brand. U+FFFC (object replacement) cannot occur in a real translation.
...
// real translations, and being distinct from BRAND_TITLE_MARKER is what lets the splice tell the two
// slots apart after the formatter has reordered them.
...
/**
 * Splices the two title slots into an already-formatted template. Stricter than [spliceBrandTitle]
 * on purpose: that one splices a brand into a *sentence*, where a repeated marker is a legitimate
 * (if odd) translation. A *title* template has exactly two slots, so anything else is damage — and
 * once a slot is missing or doubled the template can no longer tell us the intended order or
 * punctuation, which is the whole reason it exists. So a broken template is discarded whole and the
 * default title is rebuilt from the parts; patching it up piecewise would emit a title no translator
 * wrote.
 */
...
/** The app's brand mark, used wherever the canonical screen used its mascot. */
@Composable
internal fun UpgradeBrandMark(
    size: Dp,
    modifier: Modifier = Modifier,
) {
    Image(
...
// Below this much room for the copy the side-by-side split stops paying for itself: on a narrow
// screen at a large font scale the row wraps the preamble over far more lines than stacking needs,
// and comes out taller. Scaled by fontScale because the squeeze comes from text size as much as from
// screen width — at 200% font even a normal-width phone must stack.
private val HERO_MIN_TEXT_WIDTH = 150.dp

/**
 * The screen opener: brand mark and preamble in one card instead of a floating icon stacked on a
 * separate text box. Side-by-side keeps the mark at eye level with the copy it introduces and buys
 * back vertical space above the fold — but only while the copy still has room to breathe, hence the
 * stacked fallback.
 */
@Composable
internal fun UpgradeHeroCard(
    ...
    ElevatedCard(
        ...
        BoxWithConstraints(
            ...
            if (maxWidth - markSize - HERO_GAP < minTextWidth) {
                Column(
                    modifier = Modifier.fillMaxWidth(),
                    horizontalAlignment = Alignment.CenterHorizontally,
                    verticalArrangement = Arrangement.spacedBy(12.dp),
                ) {
                    UpgradeBrandMark(size = markSize)
                    Text(
                        text = text,
...
/**
 * Renders one string resource as a feature list: `•`-prefixed lines become checkmark rows, anything
 * else stays plain text. One resource rather than an array, so a translator sees the whole list in
 * context and can reorder it.
 */
@Composable
...
internal fun UpgradeLoadingBlock(
    ...
    Column(
        ...
        Text(
            text = stringResource(R.string.upgrade_loading),
            style = MaterialTheme.typography.bodyMedium,
            color = MaterialTheme.colorScheme.onSurfaceVariant,
        )
    }
...
/**
 * The "this needs Pro" marker for gated affordances elsewhere in the app (the widget/tile buttons,
 * the charge-history rows). Labelled with the flavor postfix, so the FOSS build says FOSS and the
 * Play build says Pro.
 */
@Composable
fun ProBadge(modifier: Modifier = Modifier) {
    Surface(
        modifier = modifier.testTag(UpgradeScreenTags.PRO_BADGE),
...
private const val PREVIEW_PREAMBLE =
    "Amply has no ads and doesn't sell your data. Continued development is only made possible by you."

@AmplyPreview
@Composable
private fun UpgradeScreenTitlePreview() = PreviewWrapper {
    Column(
        modifier = Modifier.padding(16.dp),
        verticalArrangement = Arrangement.spacedBy(8.dp),
    ) {
...
// one that reads as a mistake at a glance, so seeing it render "Amply Pro" in plain text documents it.
@AmplyPreview
@Composable
private fun BrandTitlePreview() = PreviewWrapper {
    Column(
        modifier = Modifier.padding(16.dp),
        verticalArrangement = Arrangement.spacedBy(8.dp),
    ) {
...
@AmplyPreview
@Composable
private fun UpgradeScreenScaffoldPreview() = PreviewWrapper {
    UpgradeScreenScaffold(
        title = AnnotatedString("Support Amply"),
        onNavigateUp = {},
    ) { paddingValues ->
...
@AmplyPreview
@Composable
private fun UpgradeScreenScaffoldTitleResPreview() = PreviewWrapper {
    UpgradeScreenScaffold(
...
@AmplyPreview
@Composable
private fun UpgradeScreenContentPreview() = PreviewWrapper {
...
@AmplyPreview
@Composable
private fun UpgradeHeaderPreview() = PreviewWrapper {
    Column(
        modifier = Modifier.padding(vertical = 16.dp),
        verticalArrangement = Arrangement.spacedBy(16.dp),
    ) {
        UpgradeHeader(markSize = 88.dp)
        UpgradeBrandMark(size = 56.dp)
    }
}

@AmplyPreview
@Composable
private fun UpgradeHeroCardPreview() = PreviewWrapper {
    Column(modifier = Modifier.padding(16.dp)) {
        UpgradeHeroCard(text = PREVIEW_PREAMBLE)
    }
}

// AmplyPreview only varies light/dark, so it can never reach the stacked branch. These two pin the
...
@Composable
private fun UpgradeHeroCardCompactPreview() = PreviewWrapper {
    Column(modifier = Modifier.padding(16.dp)) {
        UpgradeHeroCard(text = PREVIEW_PREAMBLE)
    }
}

// Both flavors tint the hero: FOSS on primaryContainer, GPLAY on secondaryContainer. Neither is the
// composable's default, so the default-colored preview above would not catch a contrast regression
// on the colors that actually ship.
@AmplyPreview
@Composable
private fun UpgradeHeroCardTintedPreview() = PreviewWrapper {
    Column(
        modifier = Modifier.padding(16.dp),
        verticalArrangement = Arrangement.spacedBy(16.dp),
    ) {
...
@AmplyPreview
@Composable
private fun UpgradeSectionCardPreview() = PreviewWrapper {
    Column(modifier = Modifier.padding(16.dp)) {
        UpgradeSectionCard(
            ...
            UpgradeFeatureList(text = "• Charge history\n• Home-screen widget")
        }
    }
}
...
@AmplyPreview
@Composable
private fun UpgradeSectionHeaderPreview() = PreviewWrapper {
    Column(
        modifier = Modifier.padding(16.dp),
        verticalArrangement = Arrangement.spacedBy(12.dp),
    ) {
        ...
        UpgradeSectionHeader(
            ...
            icon = Icons.TwoTone.AutoAwesome,
            leading = { UpgradeBrandMark(size = 24.dp) },
        )
    }
}
...
@AmplyPreview
@Composable
private fun UpgradeSectionBodyPreview() = PreviewWrapper {
    Column(modifier = Modifier.padding(16.dp)) {
        UpgradeSectionBody(
...
@AmplyPreview
@Composable
private fun UpgradeFeatureListPreview() = PreviewWrapper {
    Column(modifier = Modifier.padding(16.dp)) {
        UpgradeFeatureList(
            text = """
                A plain intro line
                • Charge history and battery statistics
                • Home-screen widget
                • Quick Settings tile long enough that this row wraps onto a second line
            """.trimIndent(),
        )
    }
}
...
@AmplyPreview
@Composable
private fun UpgradeHintTextPreview() = PreviewWrapper {
    Column(modifier = Modifier.padding(16.dp)) {
        UpgradeHintText(text = "No ads, no tracking — just you.")
    }
}

@AmplyPreview
@Composable
private fun UpgradeActionCardPreview() = PreviewWrapper {
    Column(modifier = Modifier.padding(16.dp)) {
        UpgradeActionCard(
            colors = CardDefaults.elevatedCardColors(
...
@AmplyPreview
@Composable
private fun UpgradeLoadingBlockPreview() = PreviewWrapper {
    Column(modifier = Modifier.padding(16.dp)) {
...
@AmplyPreview
@Composable
private fun UpgradeInlineStateCardPreview() = PreviewWrapper {
    Column(modifier = Modifier.padding(16.dp)) {
...
@AmplyPreview
@Composable
private fun UpgradeInlineStateCardDisabledActionPreview() = PreviewWrapper {
    Column(modifier = Modifier.padding(16.dp)) {
        UpgradeInlineStateCard(
            title = "Prices unavailable",
            body = "Google Play did not answer. This is usually temporary.",
...
private fun ProBadgePreview() = PreviewWrapper {
    Row(
        ...
        Text("Charge history", style = MaterialTheme.typography.bodyLarge)
        ProBadge()
    }
}
