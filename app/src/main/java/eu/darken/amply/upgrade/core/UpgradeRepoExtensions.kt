...
/**
 * Pro check for backend safety-net gates (e.g. the capture-enable write itself).
 *
 ...
 * upgrade screen stays snappy for free users (unlike [isProSettled], whose non-Pro path always waits
 * out its timeout). Only while billing is still connecting (GPlay cold start or reconnect, where
 * [UpgradeRepo.upgradeInfo] reports non-Pro even for paying users) does it wait, up to [timeout],
 * for the first settled Info — so a Pro user isn't bounced to the upgrade screen by the handshake
 * race. Every decision reads isPro and isSettled off the SAME Info emission, so a settle signal can
 * never pair with stale ownership. On timeout or error it falls back to the current state: the UI
 * route is recoverable, and the [isProSettled] gate remains the enforcement safety net.
 */
