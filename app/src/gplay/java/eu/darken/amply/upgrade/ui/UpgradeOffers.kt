...
// The acquisition offers box: header, offer rows, "or" divider, parity footnote.
@Composable
...
// Title and price share one line ("·"-joined in code: direction-neutral punctuation, not translatable
// copy), terms follow as body text, then the action — the terms must not repeat the button label.
@Composable
...
@AmplyPreview
@Composable
private fun LoadedOffersPreview() = PreviewWrapper {
...
@AmplyPreview
@Composable
private fun LoadedOffersNoTrialPreview() = PreviewWrapper {
    UpgradeActionCard {
        LoadedOffers(
...
// A purchase Play is still processing: the row explains itself instead of inviting a second payment.
@AmplyPreview
@Composable
private fun LoadedOffersPendingPaymentPreview() = PreviewWrapper {
    UpgradeActionCard {
        LoadedOffers(
            uiState = previewLoadedOffersState(subscriptionPending = true),
            onIap = {},
            onSubscription = {},
            onSubscriptionTrial = {},
        )
