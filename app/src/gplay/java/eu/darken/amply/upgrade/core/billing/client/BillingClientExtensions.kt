 ...
 * order ID. Debug recordings get attached to support emails by users, so anything logged here ends
 * up in an inbox and wherever it was forwarded. Everything actually useful for diagnosing an
 * entitlement problem is non-identifying, so only that is logged.
 *
