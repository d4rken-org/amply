...
import eu.darken.amply.upgrade.core.billing.BillingException
...
/**
 * A single connection attempt: emits one [BillingConnection], stays open for its lifetime, and closes
 * with the exception on setup failure or disconnect. NO retry here — `BillingManager`'s connect loop
...
open class BillingConnectionProvider @Inject constructor(
    ...
    open val connection: Flow<BillingConnection> = callbackFlow {
        ...
        // A never-answering Play (no setup callback at all) must fail this attempt into the manager's
        // backoff instead of hanging it outside any timeout.
        ...
        client.startConnection(object : BillingClientStateListener {
            override fun onBillingSetupFinished(result: BillingResult) {
                ...
                when (result.responseCode) {
                    ...
                    else -> close(BillingClientException(result))
                }
            }
