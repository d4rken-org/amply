...
/**
 * Ownership presentation for users who already own an entitlement. Subscribers without the one-time
 * purchase always see the switch offer — but LOCKED while the subscription still renews, so buying it
 * can't stack with an upcoming renewal.
 */
@Composable
internal fun UpgradeOwnershipContent(
    ...
    if (subscription != null && !ownership.hasIap) {
        ...
        // the offer is shown LOCKED with the unlock condition — a renewing subscriber must never be
        // able to stack the one-time purchase on an upcoming renewal.
        ...
        UpgradeActionCard {
            UpgradeOfferRow(
                ...
                hint = when {
                    uiState.iapPending -> stringResource(R.string.upgrade_screen_offer_payment_pending)
                    switchUnlocked -> stringResource(R.string.upgrade_screen_owned_iap_purchase_note)
                    else -> stringResource(R.string.upgrade_screen_owned_iap_locked_note)
                },
            ) {
                Button(
                    onClick = onIap,
                    // Not gated on iapEnabled: prices may have failed to load while the purchase
                    // itself would work (the billing flow re-queries details on launch). A payment
                    // Play is still processing does block it — that money is already committed.
                    enabled = switchUnlocked && uiState.busy == null && !uiState.iapPending,
                    modifier = Modifier
                        .fillMaxWidth()
    ...
    // Framed as a status re-check; support hints are offered by the failed-restore dialog only.
...
// The "you have it" moment: brand mark and congrats in one hero card at the top of the status screen,
// with the variant (subscription vs one-time) spelled out. The per-purchase cards below carry details
// and actions.
@Composable
...
/**
 * Shown on the acquisition view while the upgrade is active purely via the local grace window. Calm
 * reassurance styling, not a warning: the user has lost nothing (yet). Stage 1 confirms the upgrade
 * is intact; stage 2 (after the episode aged past the threshold) explains and offers restore.
 */
@Composable
internal fun UpgradeGraceCard(
    ...
    UpgradeSectionCard(
        ...
        // match. Once diagnostics appear the copy asks the user to act — a spinner would say "still
        // working, wait" and undercut the restore button, so the static icon returns.
        leading = if (showDiagnostics) {
            null
        } else {
            {
                CircularProgressIndicator(
...
@AmplyPreview
@Composable
private fun UpgradeOwnershipRenewingSubPreview() = PreviewWrapper {
    Column(verticalArrangement = Arrangement.spacedBy(20.dp)) {
        UpgradeOwnershipContent(
            uiState = previewLoadedState(Ownership(subscription = SubscriptionOwnership(isAutoRenewing = true))),
            onIap = {},
...
@AmplyPreview
@Composable
private fun UpgradeOwnershipNonRenewingSubPreview() = PreviewWrapper {
    Column(verticalArrangement = Arrangement.spacedBy(20.dp)) {
        UpgradeOwnershipContent(
            uiState = previewLoadedState(Ownership(subscription = SubscriptionOwnership(isAutoRenewing = false))),
            onIap = {},
            onManageSubscription = {},
            onRestore = {},
        )
...
@AmplyPreview
@Composable
private fun UpgradeOwnershipIapPreview() = PreviewWrapper {
    Column(verticalArrangement = Arrangement.spacedBy(20.dp)) {
        UpgradeOwnershipContent(
...
@AmplyPreview
@Composable
private fun UpgradeOwnershipBothRenewingPreview() = PreviewWrapper {
    Column(verticalArrangement = Arrangement.spacedBy(20.dp)) {
        UpgradeOwnershipContent(
...
@AmplyPreview
@Composable
private fun UpgradeGraceCardQuietPreview() = PreviewWrapper {
    UpgradeGraceCard(showDiagnostics = false, onRestore = {})
}

@AmplyPreview
@Composable
private fun UpgradeGraceCardDiagnosticsPreview() = PreviewWrapper {
