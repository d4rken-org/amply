...
class BillingConnection(
     ...
     * confirms absence can supersede it (null = product unknown to this app, only a complete refresh
     * may clear it).
     */
     ...
     * caches, separate event flows) exposes intermediate combinations and starves on partial
     ...
     *
     * Purchases are kept in every state (PURCHASED and PENDING alike); consumers split them, because
     * a pending purchase grants nothing but still has to be visible to the UI.
     */
    data class ReducerState(
        ...
        internal fun withEvent(
            ...
            val gen = eventGen + 1
            val entries = purchases.map { purchase ->
                OverlayEntry(
                    purchase = purchase,
                    gen = gen,
        ...
        // Never verified anything on this connection: downstream must not mistake "don't know yet"
        // for "owns nothing".
         ...
         * than the last successful query of its type (older ones were cleared), so its purchase data
         * (ack state etc.) is fresher. Dedup by `purchaseToken` (the stable purchase identity):
         * snapshot and overlay instances of the same purchase can differ in ack or purchase state,
         * so `Purchase` equality or object identity would retain both instead of letting the newer
         * one win — which is also how a PENDING purchase that later completes stops being pending.
         */
    ...
    /**
     * Fresh data straight from a Play round-trip, in COMMIT ORDER: emitted under the same lock that
     * mutates the reducer state, so a consumer can never observe a purchase event AFTER the query
     * commit that superseded it (or a stale snapshot after a newer event). Query emissions carry only
     * what the queries confirmed — never retained stale data — because consumers use this for
     * time-based bookkeeping like the grace period.
     */
    data class FreshUpdate(
         ...
         * confirmed this data. The entitlement layer stamps its grace anchor with this, so a
         * confirmation and a later connection failure are ordered by when they HAPPENED, not by when
         * each separate flow got around to processing them.
         */
        val occurredAt: Long = System.currentTimeMillis(),
    )

    // Guards state mutation + fresh emission as one atomic step. Kept a plain monitor (not a Mutex):
    // the listener path is synchronous on Play's callback thread.
    ...
    internal fun onPurchasesUpdated(result: BillingResult, purchases: Collection<Purchase>?) {
        if (result.isSuccess) {
            ...
            // Both states are ingested: PENDING is carried so the UI can show it, and the split at
            // the consumption points is what keeps it out of entitlement, acks and the grace stamp.
            val relevant = purchases.orEmpty().filter {
            ...
            synchronized(reducerLock) {
                ...
                // Only a completed purchase is fresh *entitlement* data; a pending one proves no
                // ownership and must never reach the grace bookkeeping.
                val purchased = relevant.filter { it.purchaseState == PurchaseState.PURCHASED }
    ...
    // Called by the provider when this connection ends: completes the event flows so consumers don't
    // wait on a dead connection's channels.
    internal fun close() {
    ...
    /**
     * Queries both product types and commits the result into the reducer state in ONE atomic update,
     * then returns the merged view of exactly that committed state — so the reactive [purchases] flow
     * and this return value can never disagree. Tolerant of a single product-type failure: found
     * purchases are authoritative, and an error only propagates when nothing was found AND a query
     * failed, so the caller can tell "not owned" apart from "couldn't verify".
     */
    suspend fun refreshPurchases(): PurchaseRefresh = refreshMutex.withLock {
        coroutineScope {
            ...
            val iapJob = async { queryOwnedProducts(BillingClient.ProductType.INAPP) }
            val subJob = async { queryOwnedProducts(BillingClient.ProductType.SUBS) }
    ...
    private suspend fun queryOwnedProducts(
        @BillingClient.ProductType type: String,
    ): Result<Collection<Purchase>> = try {
        Result.success(queryPurchases(type))
    ...
    private suspend fun queryPurchases(@BillingClient.ProductType type: String): Collection<Purchase> {
        ...
        // Own cancellable wrapper instead of the billing-ktx extension: a non-cancellable suspension
        // would make the timeouts around refreshes hang until Play's callback fires. The
        // onCancellation overload makes a callback racing the cancellation a no-op instead of an
        // IllegalStateException on Play's thread.
        ...
        log(TAG) {
            "queryPurchases($type): success=${billingResult.isSuccess}, message=${billingResult.debugMessage}, " +
    ...
    suspend fun acknowledgePurchase(purchase: Purchase): BillingResult {
        ...
        if (!ackResult.isSuccess) throw BillingClientException(ackResult)
        return ackResult
    }
    ...
    suspend fun querySkus(vararg skus: Sku): Collection<SkuDetails> {
        ...
        // with suspendCoroutine a missing Play callback keeps the timeout suspended indefinitely.
        val (result, queryResult) = suspendCancellableCoroutine<Pair<BillingResult, QueryProductDetailsResult>> { continuation ->
            client.queryProductDetailsAsync(params) { result, queryResult ->
                if (continuation.isActive) continuation.resume(result to queryResult) { _, _, _ -> }
            }
        }
        ...
        // A malformed product ID is OUR configuration defect and stays a hard error. The
    ...
    suspend fun launchBillingFlow(activity: Activity, sku: Sku, targetOffer: Sku.Subscription.Offer?): BillingResult {
        ...
        val params = BillingFlowParams.newBuilder().apply {
            val productDetail = BillingFlowParams.ProductDetailsParams.newBuilder().apply {
                ...
                if (sku is Sku.Subscription && targetOffer != null) {
                    ...
                    val offer = data.details.subscriptionOfferDetails?.singleOrNull { targetOffer.matches(it) }
                    if (offer == null) {
                        log(TAG, WARN) {
        ...
        log(TAG) { "launchBillingFlow(sku=$sku): code=${result.responseCode}, message=${result.debugMessage}" }
        if (!result.isSuccess) {
            // Same merchandising state as at query time, just reported by the launch instead.
    ...
    companion object {
        ...
        // authoritatively supersede them. Unknown products stay untyped (cleared only by a complete
        // refresh).
