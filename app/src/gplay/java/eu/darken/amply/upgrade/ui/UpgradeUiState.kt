...
internal sealed interface GplayUpgradeUiState {
    ...
    data class Loaded(
        ...
        /** A subscription purchase is awaiting payment approval — nothing is owned yet. */
        val subscriptionPending: Boolean = false,
        /** The one-time purchase is awaiting payment approval — nothing is owned yet. */
        val iapPending: Boolean = false,
    ) : GplayUpgradeUiState {
        val anyPending: Boolean
            get() = subscriptionPending || iapPending
    }
}

/**
 * The ONE entitlement operation currently running. Purchases and restores talk to the same Play
 * account state, so they are mutually exclusive by construction: a single slot (instead of
 * independent verifying/restoring flags) makes "which one is busy" unambiguous for both the arbiter
 * in the ViewModel and the spinner placement in the UI.
 */
...
// The upgrade is active purely via the local grace window (no owned purchase). Stage 1 shows a quiet
