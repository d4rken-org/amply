...
The Google Play version of Amply offers an optional paid upgrade. Purchases are handled entirely by Google Play; Amply never receives or stores payment details, and has no server of its own to send them to. To answer whether the upgrade is active, the app asks Google Play about the purchases on the Google account that Google Play is using for it. Locally, Amply keeps only the bookkeeping needed to keep the upgrade working during a Google Play outage: when a purchase was last confirmed and which product it was. That record stays on the device. The F-Droid/GitHub version contains no billing code at all; its unlock is a local record written after a visit to the developer's GitHub Sponsors page, and it transmits nothing.
