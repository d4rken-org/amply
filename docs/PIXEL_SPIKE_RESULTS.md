...
- **Reconnect gesture.** With the gesture enabled and the phone held at the 80% limit, the negative test (unplug, wait >10 s, replug) correctly did nothing. The positive test (unplug, replug within 10 s) drove the documented state machine `ARMED → WAITING_FOR_RECONNECT → TRIGGER` and started a one-time full-charge session. The gesture's arming notification is intentionally IMPORTANCE_LOW (silent, no heads-up); it is posted and permitted, just unobtrusive by design.
...
Still outstanding for a full physical matrix: an at-threshold below-80 % approach (hold at 80 % and charge-past on both transports), the widget under Shizuku-only, the 24-hour safety timeout (impractical at real duration; a debug-shortened test is the intended approach), and the Pixel XL / Android 10 unsupported pass.
