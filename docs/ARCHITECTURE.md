...
- `diagnostics/core` and `diagnostics/ui`: the "Help add support" contribution wizard — read-only, multi-mode setting-discovery for unsupported devices, with an on-device privacy review
...
First run presents the product intent across two pages: a welcome page and a caveat page covering device-by-device support and the one-time access setup. The interactive WSS/Shizuku setup guide is not shown during onboarding — it is rendered on the dashboard whenever durable WSS is absent. Settings use an index plus focused General, Support, and Acknowledgements sub-screens; the changelog, privacy policy, and project-help destinations are external links (`AmplyLinks` holds the project URLs). The "Help add support" contribution wizard is reachable from both the Settings hub and the unsupported-device dashboard card whenever the device is contribution-eligible (`contributionWanted`); it guides a non-technical user through Shizuku setup, labeled multi-mode capture (each capture is a full labeled snapshot; a per-key matrix is derived), an on-device two-stage privacy review (known charge keys auto-included, every other changed row — key name and values — redacted and opt-in only), and delivery as a size-guarded prefilled GitHub issue with copy/email fallbacks. Raw snapshots stay on-device (the ViewModel owns them; a generation counter discards a capture that returns after the user leaves), and the pure report builder only ever sees the reviewed, sanitized model. The report is explicitly candidate discovery, not adapter qualification (see the qualification ledger in `rules/privileged-access.md`).
